...
extern "C" __global__ void f(const ck::half_t* a0, const ck::half_t* b0, const ck::half_t* b1, ck::half_t* e1, ck::half_t* d0) {
    using G = ${template};
    ...
    if (desc.IsValid())
    {
        ${template}::Run(desc,
               a0,
               b0,
               ck::make_tuple(static_cast<const ck::half_t*>(d0)),
               b1,
               ck::make_tuple(),
               e1);
    }
...
TEST_CASE(test_problem_kernel)
{
    ...
    auto d0 = to_gpu(generate_buffer<half>(1024 * 1024, 4));

    prob.D0sTrans.push_back(false);
    prob.D0sDataType.push_back(ck::host::DataType::Half);
    prob.CDE0ElementOp = "ck::tensor_operation::element_wise::Add";
    ...
    for(auto i = 0; i < solutions.size(); ++i)
    {
        ...
        k.launch(nullptr, blocks_per_batch * block_size, block_size)(
            a.data(), b.data(), b1.data(), c.data(), d0.data());
